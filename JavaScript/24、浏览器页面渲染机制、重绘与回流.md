...
1、Rendering Tree渲染树不等同于DOM树，渲染树只包括需要显示的节点和这些节点样式信息（如display: none不会在渲染树上）

2、CSS 的 Rule Tree主要是为了完成匹配并把CSS Rule附加上Rendering Tree上的每个Element（也就是每个Frame）
...
浏览器将生产的DOM树和CSSOM树组合为渲染树Rendering Tree
...
浏览器生成渲染树Rendering Tree后，会根据渲染树布局（也叫回流）。
