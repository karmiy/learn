...
解决方法：使用 key 标识，让订阅者只订阅自己感兴趣的内容
    ...
    salesOffices.trigger = function(key, ...args) { // 发布消息
    ...
    // 测试
    salesOffices.listen('squareMeter88', function(price) { // A 订阅消息
        console.log('A: 价格= ' + price);
    });

    salesOffices.listen('squareMeter110', function(price) { // B 订阅消息
        console.log('B: 价格= ' + price);
    });
...
所以我们需要给添加 remove 方法：
    ...
    salesOffices.listen('squareMeter88', fn_a);
    salesOffices.trigger('squareMeter88', 2000000); // 输出: A: 价格= 2000000

    salesOffices.remove('squareMeter88', fn_a);
    salesOffices.trigger('squareMeter88', 2000000); // 取消了订阅，无输出
